...
class AppIconWidget extends StatelessWidget {
  ...
  const AppIconWidget({Key? key, this.size = 24.0}) : super(key: key);
...
class AppIconPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()
      ..color = const Color(0xFF2196F3) // Azul claro como el icono descrito
    ...
    for (int i = 0; i < 3; i++) {
      ...
      if (i < 2) {
        ...
        path.lineTo(checkCenter.dx - checkSize * 0.1, checkCenter.dy + checkSize * 0.2);
        path.lineTo(checkCenter.dx + checkSize * 0.3, checkCenter.dy - checkSize * 0.2);
