  ...
  Future<Map<String, int>> importData(String jsonString, BuildContext context) async {
    try {
      ...
      if (data['subjects'] != null) {
        ...
        // Se trae la lista una sola vez (no en cada iteración) y se actualiza
        // en memoria a medida que se agregan materias, para seguir evitando
        // duplicados dentro del mismo backup sin repetir la consulta de red.
        final existingSubjects = await _supabaseService.getSubjects();
        for (final subjectData in subjectsList) {
          try {
            ...
            if (!exists) {
              await _supabaseService.addSubject(subject);
