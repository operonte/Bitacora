...
class LocalCacheService {
  ...
  Future<void> initialize() async {
    ...
    try {
      ...
      _initialized = true;
      print('✅ LocalCacheService inicializado');
    } catch (e) {
      print('❌ Error inicializando LocalCacheService: $e');
      rethrow;
    }
  }
  ...
  void _onConnectivityChanged(ConnectivityResult result) {
    ...
    print('🌐 Conectividad cambiada: ${hasInternet ? "Online" : "Offline"}');
  }
  ...
  Future<void> cacheTask(Task task) async {
    ...
    await _metadataBox?.put('last_task_update_${task.id}', DateTime.now().millisecondsSinceEpoch);
  }
  ...
  Future<void> cacheTasks(List<Task> tasks) async {
    ...
    await _metadataBox?.put('last_tasks_sync', DateTime.now().millisecondsSinceEpoch);
    print('💾 ${tasks.length} tareas guardadas en caché local');
  }
  ...
  List<Task> getCachedTasks() {
    ...
    return _tasksBox?.values
        .map((data) => Task.fromMap(Map<String, dynamic>.from(data), data['id'] as String?))
        .toList() ?? [];
  }
  ...
  Future<void> cacheSubjects(List<Subject> subjects) async {
    ...
    await _metadataBox?.put('last_subjects_sync', DateTime.now().millisecondsSinceEpoch);
    print('💾 ${subjects.length} materias guardadas en caché local');
  }
  ...
  List<Subject> getCachedSubjects() {
    ...
    return _subjectsBox?.values
        .map((data) => Subject.fromMap(Map<String, dynamic>.from(data), data['id'] as String?))
        .toList() ?? [];
  }
  ...
  Future<void> markPendingSync(String entityType, String entityId, String operation) async {
    final pending = _metadataBox?.get('pending_sync', defaultValue: <Map>[]) as List? ?? [];
    pending.add({
      'type': entityType,
      'id': entityId,
  ...
  Future<void> clearAllCache() async {
    ...
    print('🧹 Caché local limpiado completamente');
  }
