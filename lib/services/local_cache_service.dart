...
class LocalCacheService {
  ...
  /// Guarda múltiples tareas en caché (sobrescribiendo totalmente la caja para purgar la caché obsoleta)
  Future<void> replaceCachedTasks(List<Task> tasks) async {
    if (!_initialized) return;
    ...
    Logger.info(
      '${tasks.length} tareas actualizadas limpiamente en caché local',
      tag: 'Cache',
    );
  }
  ...
  /// Reemplaza la cola de pendientes por [items] (p.ej. solo los que fallaron
  /// en una ronda de sincronización), en vez de vaciarla por completo.
  Future<void> replacePendingSync(List<Map<String, dynamic>> items) async {
    await _metadataBox?.put('pending_sync', items);
  }
