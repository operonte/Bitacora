...
|---|---|---|---|
| **2.27.1** | `63ab615` | 2026-09-09 | En "Hoy" del inicio, tocar una reunión conecta directo si tiene enlace en vez de abrir siempre la pantalla de editar (pasaba incluso con reuniones solo compartidas, no propias). Detectado durante una auditoría de seguridad de la base (Supabase) que no encontró huecos explotables, pero sí un bug pendiente de corregir por separado: `set_task_grade`/`set_task_teacher_comment` siguen exigiendo ser quien creó la tarea, sin la misma ampliación que ya tiene `get_task_submission_status` desde v2.26.0 — un docente que co-dicta la asignatura puede ver las entregas pero no guardar nota/comentario. **Sin probar en la app corriendo** — solo compilación y análisis estático. |
| 2.27.0 | `4d75f09` | 2026-09-09 | Foto de portada del perfil acomodable arrastrándola (como Facebook), tanto al subir una nueva como sobre una ya puesta — corrige de paso que el perfil público nunca mostraba la portada. Género y Situación sentimental pasan de texto libre a listas desplegables. "Carrera anterior u ocupación" se renombra a "Estudios". Tablas de Notas y Asistencia por carrera más compactas. **Sin probar en la app corriendo** — solo compilación, análisis estático, y el GRANT de la columna nueva verificado con un UPDATE simulado contra la base real. Publicado como APK, GitHub Release y desplegado en la web.
| **2.26.1** | `4f39826` | 2026-09-09 | Corrige "permission denied" al guardar teléfono/redes/intereses/carrera anterior/profesión/portada del perfil (faltaba el GRANT de esas columnas nuevas de v2.26.0, nunca se habían guardado de verdad — bug reportado en vivo). Borrar o reemplazar una historia ahora elimina el archivo de Drive de verdad, no solo revoca el link. Tope de video de historias subido de 20 a 80MB (20 rechazaba un video real de ~30s). "Quién la debe" en tarjetas separadas. Mascota tocable siempre con frase en primera persona + nivel "enojada", ícono zen duplicado sacado de Home docente. **Sin probar en la app corriendo** — solo compilación, análisis estático, y el GRANT verificado con un UPDATE simulado contra la base real. |
| 2.26.0 | `a8ecc9d` | 2026-09-09 | Notificaciones llevan a la sección correspondiente al tocarlas, Historias de perfil (foto/video, se borran solas a las 48 h, verificado contra la base real), toda la app pasa de voseo a tuteo formal, evaluar una tarea compartida ya no exige haberla creado, anuncios acotados a asignaturas propias, Archivos de alumnos agrupado por asignatura con buscador, foto de portada y 5 campos nuevos en el perfil con contacto tocable, logo por carrera, Mis notas/Mi asistencia en tabla compacta por semestre. **Sin probar en la app corriendo** — solo compilación, análisis estático, y las políticas de Historias (propia, compartida con la carrera, vencimiento a 48 h) y de evaluación ampliada verificadas con consultas simuladas contra la base real. |
| 2.25.0 | `83a9824` | 2026-09-09 | Material docente limitado a las asignaturas que el docente imparte, corrige que Drive negara el acceso a un archivo ya visible en "Archivos de alumnos", "Mis archivos" no aparece para docente, Mi Perfil sin "Unirse"/"Salir de la carrera" para docente, botón "Ver notas y asistencia" desde el perfil de un alumno, 5 campos nuevos en Información personal, resumen de asistencia más compacto y campo Nota solo numérico. **Sin probar en la app corriendo** — solo compilación, análisis estático y el RPC de Drive/archivos verificado con consultas simuladas contra la base real (acceso concedido a la cuenta docente para el archivo correcto, rechazado a una de alumno). |
